...
function getStoredNews(forceRefresh = false): Article[] {
        ...
        if (stored) {
            try {
                ...
                if (Array.isArray(parsed)) {
                    newsData = parsed; // Update in-memory cache
                    return parsed;
                }
            } catch (e) {
                console.error("Failed to parse news from localStorage", e);
                // Fallback to in-memory data if parsing fails
            }
        }
    }
  }
...
if (typeof window !== 'undefined') {
    ...
    if(stored) {
        ...
        storeNews(newsData); // Store initial data if nothing is in local storage
    }
}

export async function getNews(): Promise<Article[]> {
  await new Promise(resolve => setTimeout(resolve, 50));
  // Force refresh from localStorage to get the latest data on the client.
  const currentNews = getStoredNews(true); 
  return currentNews;
}
...
export async function updateNews(articleId: number, updateData: Partial<NewArticle>): Promise<Article> {
    await new Promise(resolve => setTimeout(resolve, 100));
    let currentNews = getStoredNews(true);
...
export async function deleteNews(articleId: number): Promise<{ success: true }> {
    await new Promise(resolve => setTimeout(resolve, 100));
    let currentNews = getStoredNews(true);
    const initialCount = currentNews.length;
    const updatedNews = currentNews.filter(a => a.id !== articleId);

    if (initialCount === updatedNews.length) {
        // To avoid throwing an error when an item is already deleted by another process
        console.warn(`Article with id ${articleId} not found for deletion, it might have been already deleted.`);
    }
