...
export function addNews(articleData: NewArticle): Article {
...
export function updateNews(articleId: number, updateData: Partial<NewArticle>): Article {
...
export function deleteNews(articleId: number): { success: true } {
    const initialCount = newsData.length;
    newsData = newsData.filter(a => a.id !== articleId);

    if (initialCount === newsData.length) {
        throw new Error(`Article with id ${articleId} not found for deletion.`);
    }

    return { success: true };
}

export function deleteAllNews(): { success: true, count: number } {
